...
def fit_langley(langley, weighted = False):
    """
...
class deprecated_SP02RawData(object):
    ...
    @property
    def langleys_am(self):
        if isinstance(self._langleys_am, type(None)):
            self._get_langley_from_raw()
        return self._langleys_am
    
    @property
    def langleys_pm(self):
        if isinstance(self._langleys_pm, type(None)):
            self._get_langley_from_raw()
        return self._langleys_pm

    @property
    def langley_residual_correlation_am(self):
        if isinstance(self._langley_fitres_am, type(None)):
            self._langley_fitres_am = self._fit_langles(which = 'am')
        return self._langley_fitres_am
    
    @property
    def langley_fitres_pm(self):
        if isinstance(self._langley_fitres_pm, type(None)):
            self._langley_fitres_pm = self._fit_langles(which = 'pm')
        return self._langley_fitres_pm

    @property
    def langley_fitres_am(self):
        if isinstance(self._langley_fitres_am, type(None)):
            self._langley_fitres_am = self._fit_langles(which = 'am')
        return self._langley_fitres_am
    ...
    def _fit_langles(self, which):
        if which == 'am':
            langleys = self.langleys_am
        elif which == 'pm':
            langleys = self.langleys_pm
        ...
        df_resid = pd.DataFrame(resid_dict)
        resid_corr_matrix = df_resid.corr()
        corrnp.linalg.det(resid_corr_matrix)
...
#         self.tp_raw_df = raw_df.copy()
        ...
        self._langleys_am = langley_am[~langley_am.index.isna()]
        self._langleys_pm = langley_pm[~langley_pm.index.isna()]
        return True
